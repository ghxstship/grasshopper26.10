...
export function useDayOfShow(filters?: Record<string, any>) {
  const params = new URLSearchParams(filters || {});
  
  const { data, error, mutate, isLoading } = useSWR(
    `/api/compvss/dayofshow?${params}`,
    fetcher,
    {
...
export function useCreateDayOfShow() {
  ...
  const create = useCallback(async (data: Record<string, any>) => {
    ...
    try {
      const response = await fetch('/api/compvss/dayofshow', {
        method: 'POST',
