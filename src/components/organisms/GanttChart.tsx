...
export function GanttChart({
  ...
  const getTaskPosition = (task: GanttTask) => {
    ...
    const left =
      ((taskStart - startDate.getTime()) / (1000 * 60 * 60 * 24) / totalDays) *
      100;
    const width =
  ...
  const getProgressColor = (progress: number) => {
    ...
    if (progress >= 25) return "bg-black";
    return "bg-error";
  };
  const renderTask = (task: GanttTask, level: number = 0) => {
    ...
    return (
      <div key={task.id}>
        {" "}
        {/* Task Row */}{" "}
        <div className="flex border-b hover: transition-colors">
          {" "}
          {/* Task Info Column */}{" "}
          <div className="w-80 flex-shrink-0 p-4 border-r">
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                {task.assignee && (
                  <div className="mt-1"> {task.assignee} </div>
                )}{" "}
              </div>{" "}
          ...
          <div className="flex-1 relative p-4">
            {" "}
            <motion.div
              className="relative h-8 rounded-none overflow-hidden cursor-pointer"
              style={position}
  ...
  return (
    <div className="bg-black rounded-none border overflow-hidden">
      {" "}
      {/* Header */}{" "}
      <div className="flex border-b">
        ...
        <div className="flex-1 flex">
          ...
          {timelineMonths.map((month, index) => (
            <div key={index} className="flex-1 p-4 border-r last:border-r-0">
              {" "}
              <div>
                {" "}
      ...
      {/* Tasks */}{" "}
      <div className="overflow-y-auto max-h-[37.50rem]">
        {" "}
