...
import { Package, Star, Filter, Grid3x3, List, ShoppingCart, TrendingUp, Sparkles, Clock } from 'lucide-react';
...
export default function ProductsPage() {
  ...
  const renderRating = (rating: number, reviewCopy: string) => (
    <div className="flex items-center gap-2">
      <div className="flex items-center">
        {[...Array(5)].map((_, index) => (
          <Star
            key={index}
            className={`h-4 w-4 ${index < Math.floor(rating) ? 'fill-gvteway-yellow-500 text-gvteway-yellow-500' : ''}`}
          />
        ))}
      </div>
    ...
    <Card key={product.id} variant="gvteway" className="group cursor-pointer hover:scale-[1.02] transition-transform">
      ...
      <CardHeader>
        <div className="flex items-start justify-between gap-2 mb-2">
          ...
          <Badge variant="gvteway-outline">{product.category}</Badge>
        </div>
        <CardDescription className="line-clamp-2">{product.description}</CardDescription>
        <div className="mt-2">{renderRating(product.rating, `${product.rating} (${product.reviewCount})`)}</div>
      </CardHeader>
      <CardFooter className="flex flex-col gap-3">
        <div className="flex items-baseline justify-between w-full">
          <div className="flex items-baseline gap-2">
            <SubsectionHeader className="text-gvteway-red-500">${product.price}</SubsectionHeader>
            {product.originalPrice && <Caption className="line-through">${product.originalPrice}</Caption>}
          </div>
        </div>
  ...
  const renderProductListItem = (product: Product) => (
    <Card
      key={product.id}
      variant="gvteway"
      className="group cursor-pointer hover:border-gvteway-red-500/50 transition-all"
      onClick={() => handleProductClick(product.id)}
    >
          ...
          <div>
            <div className="flex items-start justify-between gap-4 mb-2">
              <div className="flex-1">
                ...
                <BodyTextSmall className="mb-3">{product.description}</BodyTextSmall>
                {renderRating(product.rating, `${product.rating} (${product.reviewCount} reviews)`)}
              </div>
            </div>
  ...
  return (
    <GvtewayLayout>
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <Card variant="gvteway">
            <CardContent className="flex items-center gap-4">
              ...
              <div>
                <SubsectionHeader>{products.filter((product) => product.new).length}</SubsectionHeader>
          ...
          <Card variant="gvteway">
            <CardContent className="flex items-center gap-4">
              ...
              <div>
                <SubsectionHeader>{products.filter((product) => product.trending).length}</SubsectionHeader>
          ...
          <Card variant="gvteway">
            <CardContent className="flex items-center gap-4">
              ...
              <div>
                <SubsectionHeader>{products.filter((product) => product.inStock).length}</SubsectionHeader>
          ...
          <CardContent>
            <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                <Button variant="gvteway-outline" size="sm" leftIcon={<Filter className="h-4 w-4" />}>
                  Filters
                </Button>
              </div>
            </div>
        ...
        <div className="mb-8">
          <Tabs tabs={categoryTabs} activeTab={activeCategory} onChange={setActiveCategory} variant="gvteway" />
        </div>
        ...
        {filteredProducts.length > 0 ? (
          <>
            {viewMode === 'grid' ? (
              ...
              <div className="space-y-4 mb-8">{filteredProducts.map(renderProductListItem)}</div>
            )}
