...
// This component calls: /Users/julianclarkson/Documents/Grasshopper26.10/api/gvteway/wallet/loyalty
      ...
      <GvtewayLayout>
        <div className="min-h-screen bg-black pt-20 flex items-center justify-center">
          <div className="text-center">
            ...
            <BodyText className="text-grey-400">Loading loyalty data...</BodyText>
          </div>
        </div>
      </GvtewayLayout>
    );
            ...
            >
              ...
              <Card variant="gvteway" className="bg-gradient-to-br from-gvteway-red-500/20 to-gvteway-blue-500/20 backdrop-blur-sm mb-8">
                <CardContent className="p-8">
                  <div className="text-center mb-6">
                    ...
                    <BodyText className="text-grey-300 text-body-sm mb-2">Your Balance</BodyText>
                    <p className="text-white mb-1">{currentPoints.toLocaleString()}</p>
                    <BodyText className="text-grey-300">Loyalty Points</BodyText>
                  </div>

                  {/* Progress to Next Tier */}
                  <div className="max-w-md mx-auto">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-body-sm text-grey-300">Progress to Gold Tier</span>
                      <span className="text-body-sm text-grey-300">{Math.round(progress)}%</span>
                    </div>
                    ...
                    <p className="text-center text-grey-400 text-body-sm mt-2">
              ...
              <div className="grid lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2">
                  <SectionHeader className="text-white mb-6">Available Rewards</SectionHeader>
                  <div className="space-y-4" role="list" aria-labelledby="rewards-heading">
                    {REWARDS.map((reward) => (
                      <Card key={reward.id} variant="gvteway" className="bg-grey-900/50 backdrop-blur-sm" role="listitem">
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                ...
                                <p className="text-grey-400 text-body-sm">{reward.points} points</p>
                              </div>
                            </div>
                            <Button
                  ...
                  {/* Points History */}
                  <SectionHeader className="text-white mb-6 mt-12">Points History</SectionHeader>
                    ...
                    <CardContent className="p-6">
                      <div className="space-y-4" role="list" aria-labelledby="history-heading">
                        {HISTORY.map((item, index) => (
                          <div key={index} className="flex items-center justify-between py-3 border-b border-grey-800 last:border-0" role="listitem">
                            <div className="flex items-center gap-3">
                              <div className={`w-2 h-2 rounded-full ${ item.type === 'earned' ? 'bg-success-light0' : 'bg-error' }`} aria-hidden="true" />
                              <div>
                                <p className="text-white">{item.action}</p>
                                <p className="text-grey-400 text-body-sm flex items-center">
                                  <Calendar className="w-3 h-3 mr-1" aria-hidden="true" />
                                  {item.date}
                                </p>
                              </div>
                            </div>
                            <span className={`${ item.type === 'earned' ? 'text-success' : 'text-error' }`}>
                              {item.points}
                            </span>
                          </div>
                        ))}
                ...
                <div className="space-y-6">
                  ...
                  <Card variant="gvteway" className="bg-grey-900/50 backdrop-blur-sm">
                    ...
                    <CardContent>
                      <div className="text-center mb-4">
                        <div className="inline-flex items-center justify-center w-16 h-16 bg-grey-700 rounded-full mb-3" aria-hidden="true">
                          <Star className="w-8 h-8 text-grey-400" aria-hidden="true" />
                        </div>
                        <SubsectionHeader className="text-white mb-1">Silver</SubsectionHeader>
                    ...
                    <CardContent>
                      <div className="space-y-3 text-body-sm">
                        <div className="flex items-start">
                          ...
                          <div>
                            <BodyText className="text-white">Purchase Tickets</BodyText>
                            <BodyText className="text-grey-400">Earn 1 point per $1 spent</BodyText>
                          </div>
                        </div>
                        <div className="flex items-start">
                          <Award className="w-4 h-4 text-gvteway-red-500 mr-2 mt-1 flex-shrink-0" />
                          <div>
                            <BodyText className="text-white">Attend Events</BodyText>
                            <BodyText className="text-grey-400">+100 points per check-in</BodyText>
                          </div>
                        </div>
                        <div className="flex items-start">
                          <Award className="w-4 h-4 text-gvteway-red-500 mr-2 mt-1 flex-shrink-0" />
                          <div>
                            <BodyText className="text-white">Refer Friends</BodyText>
                            <BodyText className="text-grey-400">+500 points per referral</BodyText>
                          </div>
                        </div>
                      </div>
                    </CardContent>
                  ...
                  <Card variant="gvteway" className="bg-gradient-to-br from-gvteway-red-500/20 to-gvteway-blue-500/20 backdrop-blur-sm">
                    <CardContent className="p-6 text-center">
                      ...
                      <BodyText className="text-grey-300 text-body-sm mb-4">
