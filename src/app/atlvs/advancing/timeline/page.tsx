...
// This component calls: /Users/julianclarkson/Documents/Grasshopper26.10/api/atlvs/advancing/timeline
          ...
          <Card variant="atlvs" className="bg-grey-900/50">
            <CardHeader>
              <div className="text-center py-12">
                ...
                <SubsectionHeader className="mb-2">Failed to Load Timeline</SubsectionHeader>
            ...
            <CardHeader>
              ...
              {events.length === 0 ? (
                <div className="text-center py-12 text-grey-400">
                  ...
                  <BodyText >No timeline events yet</BodyText>
                </div>
              ) : (
                  ...
                  {events.map((event: TimelineEvent, index: number) => (
                    <div key={event.id} className="flex items-start gap-4" role="listitem">
                      <div className="flex flex-col items-center">
                        <div className={`w-4 h-4 rounded-full ${ event.status === 'completed' ? 'bg-atlvs-green-500' : event.status === 'pending' ? 'bg-warning' : 'bg-info' }`} aria-hidden="true" />
                        {index < events.length - 1 && (
                          <div className="w-0.5 h-16 bg-grey-700 my-2" aria-hidden="true" />
                        )}
                      </div>
                      <div className="flex-1 pb-8">
                        <div className="flex items-start justify-between mb-2">
                          <div>
                            ...
                            <div className="text-body-sm text-grey-400">{event.date} at {event.time}</div>
                            {event.project && (
                              <div className="text-caption text-grey-500 mt-1">{event.project}</div>
                            )}
                          </div>
