...
export default function AdvancingDashboardPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'approved':
        return <Badge variant="compvss" className="bg-success-light text-success border-green-500/30">Approved</Badge>;
      case 'pending':
        return <Badge variant="compvss-outline" className="border-yellow-500/30 text-warning">Pending</Badge>;
      case 'in_review':
        return <Badge variant="compvss-outline" className="border-blue-500/30 text-info">In Review</Badge>;
      case 'rejected':
        return <Badge variant="compvss-outline" className="border-red-500/30 text-error">Rejected</Badge>;
      default:
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="border-b border-gray-800 bg-gradient-to-r from-black via-gray-950 to-black">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bebas compvss-text-gradient">Production Advancing</h1>
              <p className="text-gray-400 font-oswald mt-1">Submit and track advancing requests</p>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <Card variant="compvss" className="bg-gray-900/50">
            <CardContent className="pt-6">
              <div className="text-3xl font-bebas text-white mb-1">{stats.total}</div>
              <div className="text-sm text-gray-400 font-oswald">Total Requests</div>
            </CardContent>
          </Card>
          <Card variant="compvss" className="bg-gray-900/50">
            <CardContent className="pt-6">
              <div className="text-3xl font-bebas text-success mb-1">{stats.approved}</div>
              <div className="text-sm text-gray-400 font-oswald">Approved</div>
            </CardContent>
          </Card>
          <Card variant="compvss" className="bg-gray-900/50">
            <CardContent className="pt-6">
              <div className="text-3xl font-bebas text-warning mb-1">{stats.pending}</div>
              <div className="text-sm text-gray-400 font-oswald">Pending</div>
            </CardContent>
          </Card>
          <Card variant="compvss" className="bg-gray-900/50">
            <CardContent className="pt-6">
              <div className="text-3xl font-bebas text-info mb-1">{stats.inReview}</div>
              <div className="text-sm text-gray-400 font-oswald">In Review</div>
            </CardContent>
          </Card>
        </div>
        ...
        <motion.div
          ...
          className="mb-8"
        >
          <h2 className="text-2xl font-bebas text-white mb-4">Request Categories</h2>
                ...
                <Link href={`/compvss/advancing/category/${category.name.toLowerCase().replace(/ /g, '-')}`}>
                  <Card variant="compvss" className="bg-gray-900/50 hover:bg-gray-900/70 transition-all cursor-pointer h-full">
                    <CardContent className="pt-6 text-center">
                      ...
                      <p className="font-oswald text-white text-sm mb-2">{category.name}</p>
