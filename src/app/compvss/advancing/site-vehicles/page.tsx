...
// This component calls: /Users/julianclarkson/Documents/Grasshopper26.10/api/compvss/advancing/site-vehicles

export default function SiteVehiclesAdvancingPage() {
  const router = useRouter();
  const { errors, isSubmitting, handleSubmit, handleSaveDraft } = useAdvancingForm({
    category: 'site-vehicles',
  ...
  return (
    <CompvssLayout>
      <ContentLayout
        title="Site Vehicles Request"
        description="Site vehicles and transport"
          ...
          <Card variant="compvss" className="bg-grey-900/50">
            <CardHeader>
              <CardTitle className="text-white flex items-center gap-2">
                <Truck className="w-5 h-5 text-compvss-cyan-500" />
                Site Vehicles Details
              </CardTitle>
              <CardDescription className="text-grey-400">
                Standard advancing request form - Site Vehicles category
              </CardDescription>
            </CardHeader>
            <CardContent>
              {errors.submit && (
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <FormField label="Vehicle Type" required>
                  <Select className="bg-black/50 border-compvss-cyan-500/30" variant="compvss">
                    ...
                    <option value="golf_cart">Golf Carts</option>
                    <option value="atv">ATVs/UTVs</option>
                    <option value="forklift">Forklifts</option>
                    <option value="truck">Pickup Trucks</option>
                    <option value="van">Cargo Vans</option>
                    <option value="other">Other</option>
                  </Select>
                </FormField>

                <FormField label="Quantity" required>
                  <Input type="number" placeholder="Number of vehicles" className="bg-black/50 border-compvss-cyan-500/30" variant="compvss" required />
                </FormField>

                <FormField label="Detailed Requirements" required>
                  <Textarea placeholder="Specific vehicle needs, operator requirements, usage schedule..." className="bg-black/50 border-compvss-cyan-500/30" variant="compvss" required />
                </FormField>

                <FormField label="Budget Estimate">
                  <Input type="text" placeholder="e.g., $2,000" className="bg-black/50 border-compvss-cyan-500/30" variant="compvss" />
                </FormField>
