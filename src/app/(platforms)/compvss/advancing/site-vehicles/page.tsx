/**
 * Site Vehicles Advancing Page - UI Rebuild
 * Manage site vehicle requests (golf carts, trucks, forklifts)
 */
...
export default function SiteVehiclesPage() {
  ...
  React.useEffect(() => {
    const fetchRequests = async () => {
      try {
        ...
        const response = await apiClient.get<{ requests: VehicleRequest[] }>(
          '/api/compvss/advancing/site-vehicles'
        );
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="flex items-center justify-between mb-8">
          <div>
            <H1 className="mb-2">Site Vehicles</H1>
            <Body className="text-gray-600">
              Golf carts, trucks, forklifts, and transport vehicles
            </Body>
          </div>
          <Link href="/compvss/advancing/new?type=site-vehicles">
            <Button variant="compvss">New Vehicle Request</Button>
          </Link>
        </div>
            ...
            <Link key={request.id} href={`/compvss/advancing/requests/${request.id}`}>
              <Card className="hover:shadow-[4px_4px_0_0_rgba(0,0,0,1)] hover:translate-x-[-2px] hover:translate-y-[-2px] transition-all cursor-pointer">
                <CardHeader>
                  ...
                  <CardTitle className="capitalize">{request.vehicleType}</CardTitle>
          ...
          {requests.length === 0 && (
            <Card variant="compvss" className="col-span-full">
              <CardContent className="p-12 text-center">
                <Body className="text-gray-500 mb-4">No vehicle requests yet</Body>
                <Link href="/compvss/advancing/new?type=site-vehicles">
