/**
 * Site Infrastructure Advancing Page - UI Rebuild
 * Manage site infrastructure requests (fencing, barriers, structures)
 */
...
export default function SiteInfrastructurePage() {
  ...
  const [requests, setRequests] = React.useState<InfrastructureRequest[]>([]);
  ...
  React.useEffect(() => {
    const fetchRequests = async () => {
      try {
        ...
        const response = await apiClient.get<{ requests: InfrastructureRequest[] }>(
          '/api/compvss/advancing/site-infrastructure'
        );
        if (response.data?.requests) {
          setRequests(response.data.requests);
        }
      } catch (error) {
        console.error('Failed to fetch infrastructure requests:', error);
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="flex items-center justify-between mb-8">
          <div>
            <H1 className="mb-2">Site Infrastructure</H1>
            <Body className="text-gray-600">
              Fencing, barriers, stages, and structural requests
            </Body>
          </div>
          <Link href="/compvss/advancing/new?type=site-infrastructure">
            <Button variant="compvss">New Infrastructure Request</Button>
          </Link>
        </div>
            ...
            <Link key={request.id} href={`/compvss/advancing/requests/${request.id}`}>
              <Card className="hover:shadow-[4px_4px_0_0_rgba(0,0,0,1)] hover:translate-x-[-2px] hover:translate-y-[-2px] transition-all cursor-pointer">
                <CardHeader>
                  ...
                  <CardTitle className="capitalize">{request.infrastructureType}</CardTitle>
          ...
          {requests.length === 0 && (
            <Card variant="compvss" className="col-span-full">
              <CardContent className="p-12 text-center">
                <Body className="text-gray-500 mb-4">No infrastructure requests yet</Body>
                <Link href="/compvss/advancing/new?type=site-infrastructure">
