/**
 * Casual Adventures Page - UI Rebuild
 * Browse and book casual, laid-back experiences
 */
...
export default function CasualAdventuresPage() {
  const [adventures, setAdventures] = React.useState<CasualAdventure[]>([]);
  ...
  React.useEffect(() => {
    const fetchAdventures = async () => {
      try {
        setLoading(true);
        const params: Record<string, string> = { type: 'CASUAL' };
        if (searchQuery) params.search = searchQuery;
        
        const response = await apiClient.get<{ adventures: CasualAdventure[] }>('/api/adventures', {
          params,
        });
        ...
        console.error('Failed to fetch casual adventures:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="mb-12">
          <H1 className="mb-4">Casual Adventures</H1>
          <Body className="text-gray-600">
            Relaxed, easy-going experiences perfect for unwinding and having fun.
          </Body>
        </div>

        <div className="mb-8">
          <SearchBar
            placeholder="Search casual adventures..."
            value={searchQuery}
        ...
        {adventures.length === 0 ? (
          <Card>
            <CardContent className="py-24 text-center">
              <H3 className="mb-4">No casual adventures available</H3>
              <Body className="text-gray-600">
                Check back soon for new casual experiences.
              </Body>
            </CardContent>
          </Card>
        ) : (
              ...
              <Card key={adventure.id}>
                ...
                <CardHeader>
                  <div className="flex items-start justify-between mb-2">
                    <Badge>{adventure.difficulty}</Badge>
