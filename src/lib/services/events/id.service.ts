...
export class EventsService {
  ...
  async update(params: string | { where: { id: string } | { slug: string }; data: any; include?: any }, data?: any) {
    if (typeof params === 'string') {
      return await prisma.event.update({ where: { id: params }, data: data! });
    }
    return await prisma.event.update(params as any);
  }
  ...
  async delete(params: string | { where: { id: string } | { slug: string } }) {
    ...
    return await prisma.event.delete(params as any);
  }
}
